...
class Item:
    def __init__(self, name, cost, rect):
        ...
        self.purchased = False  # Track if the item is purchased and the method

    def can_afford(self, player_money):
        return player_money >= self.cost# uses return to money to buy stuff




    def purchase(self):
        global money# I honestly have no idea what this does it may notbe needed
...
# Create items
warrior_monkey = Item("Warrior Monkey", 80, pygame.Rect(50, 200, button_width, button_height))#example of inheritance in code
...
def draw_button(rect, text):
    ...
    text_surface = font.render(text, True, (255, 255, 255))  # Most of this stuff is self explanitory
    ...
    while active:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:#Check if user closed window
                pygame.quit()
                sys.exit()
            if event.type == pygame.KEYDOWN:#Used to handle typing in python
                if event.key == pygame.K_RETURN:
                    active = False  # End input or writting when Enter is pressed
                elif event.key == pygame.K_BACKSPACE:
                    user_input = user_input[:-1]  # Remove the last character because idk really know why but i just need to do it just like when we were learning how to divide and stufff
                else:
                    user_input += event.unicode  # Add the character typed to the input
        return user_input

# Makes text that fit into a box
def draw_text_in_box(text, rect, font, color):
    words = text.split(" ")
    lines = []
    current_line = ""
   
    # Looop through each word to make sure it fits in box
    for word in words:
    ...
    # How the black texts box are made with text
...
# Function for draw the money counter in the top-left corner
...
# Main game starts, menu screen
running = True
current_screen = "menu"  # Start at the main menu screen
user_input = ""
collect_banana_completed = False  #All bolean
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if current_screen == "jungle" and continue_button_rect.collidepoint(mouse_x, mouse_y):
                ...
                collect_banana_completed = True  # Mark that collect banana game is completed with bolean
            ...
            if collect_banana_completed and advance_button_rect.collidepoint(mouse_x, mouse_y):
                ...
                current_screen = "new_background"  # Change to new background (new.jpg) the guy with 2 options

            # Swutches to jungle screens after items are chosen😊😊😊😊
    ...
    if current_screen == "menu":
        ...
        # Draw message box on the jungle game backround
        ...
        # Continue Button which starts game
        draw_button(continue_button_rect, "Continue")

    elif current_screen == "new_background":
        # After advancing by exiting out window it changes to dis
        screen.fill((255, 255, 255))
        ...
        banana_status_surface = font.render(banana_status_text, True, (255, 255, 255))
